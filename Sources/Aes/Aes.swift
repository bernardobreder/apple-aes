...
public class AESCipher: AESBase {
    ...
    private lazy var expandedKey: Array<Array<UInt32>> = self.expandKey(self.key)
    
    private lazy var expandedKeyInv: Array<Array<UInt32>> = self.expandKeyInv(self.key)
    ...
    public func encrypt(data: Data) -> Data {
        let chunks = PKCS7().add(bytes: Array(data), blockSize: AESBase.blockSize).chunks()
        return Data(bytes: iv) + Data(bytes: blockMode.encrypt(blocks: chunks, iv: iv, cipherOperation: encrypt(block:)))
    }
    
    public func decrypt(data: Data) -> Data {
        let iv = Array(data.subdata(in: data.startIndex ..< data.index(data.startIndex, offsetBy: AESBase.blockSize)))
        let blocks = Array(data.subdata(in: data.index(data.startIndex, offsetBy: AESBase.blockSize) ..< data.endIndex)).chunks()
        return Data(bytes: PKCS7().remove(bytes: blockMode.decrypt(blocks: blocks, iv: iv, cipherOperation: decrypt(block:)), blockSize: AESBase.blockSize))
    }
    
    func encrypt(bytes: [UInt8]) -> [UInt8] {
        let finalBytes = PKCS7().add(bytes: bytes, blockSize: AESBase.blockSize)
        let blocks = finalBytes.chunks()
        return blockMode.encrypt(blocks: blocks, iv: iv, cipherOperation: encrypt(block:))
    }
    ...
    private func encrypt(block: [UInt8]) -> [UInt8] {
        ...
        for r in 0 ..< AESBase.rounds - 1 {
            t[0] = b[0] ^ rk[r][0]
            t[1] = b[1] ^ rk[r][1]
            t[2] = b[2] ^ rk[r][2]
            t[3] = b[3] ^ rk[r][3]
            
            let lb00 = AESCipher.T0[Int(t[0] & 0xFF)]
            let lb01 = AESCipher.T1[Int((t[1] >> 8) & 0xFF)]
            let lb02 = AESCipher.T2[Int((t[2] >> 16) & 0xFF)]
            let lb03 = AESCipher.T3[Int(t[3] >> 24)]
            b[0] = lb00 ^ lb01 ^ lb02 ^ lb03
            
            let lb10 = AESCipher.T0[Int(t[1] & 0xFF)]
            let lb11 = AESCipher.T1[Int((t[2] >> 8) & 0xFF)]
            let lb12 = AESCipher.T2[Int((t[3] >> 16) & 0xFF)]
            let lb13 = AESCipher.T3[Int(t[0] >> 24)]
            b[1] = lb10 ^ lb11 ^ lb12 ^ lb13
            
            let lb20 = AESCipher.T0[Int(t[2] & 0xFF)]
            let lb21 = AESCipher.T1[Int((t[3] >> 8) & 0xFF)]
            let lb22 = AESCipher.T2[Int((t[0] >> 16) & 0xFF)]
            let lb23 = AESCipher.T3[Int(t[1] >> 24)]
            b[2] = lb20 ^ lb21 ^ lb22 ^ lb23
            
            let lb30 = AESCipher.T0[Int(t[3] & 0xFF)]
            let lb31 = AESCipher.T1[Int((t[0] >> 8) & 0xFF)]
            let lb32 = AESCipher.T2[Int((t[1] >> 16) & 0xFF)]
            let lb33 = AESCipher.T3[Int(t[2] >> 24)]
            b[3] = lb30 ^ lb31 ^ lb32 ^ lb33
        }
        
        // last round
        let r = AESBase.rounds - 1
        ...
        // rounds
        b[0] = F1(t[0], t[1], t[2], t[3]) ^ rk[AESBase.rounds][0]
        b[1] = F1(t[1], t[2], t[3], t[0]) ^ rk[AESBase.rounds][1]
        b[2] = F1(t[2], t[3], t[0], t[1]) ^ rk[AESBase.rounds][2]
        b[3] = F1(t[3], t[0], t[1], t[2]) ^ rk[AESBase.rounds][3]
    ...
    private func decrypt(block: [UInt8]) -> [UInt8] {
        ...
        for r in (2...AESBase.rounds).reversed() {
            t[0] = b[0] ^ rk[r][0]
            t[1] = b[1] ^ rk[r][1]
            t[2] = b[2] ^ rk[r][2]
            t[3] = b[3] ^ rk[r][3]
            
            let b00 = AESCipher.T0_INV[Int(t[0] & 0xFF)]
            let b01 = AESCipher.T1_INV[Int((t[3] >> 8) & 0xFF)]
            let b02 = AESCipher.T2_INV[Int((t[2] >> 16) & 0xFF)]
            let b03 = AESCipher.T3_INV[Int(t[1] >> 24)]
            b[0] = b00 ^ b01 ^ b02 ^ b03
            
            let b10 = AESCipher.T0_INV[Int(t[1] & 0xFF)]
            let b11 = AESCipher.T1_INV[Int((t[0] >> 8) & 0xFF)]
            let b12 = AESCipher.T2_INV[Int((t[3] >> 16) & 0xFF)]
            let b13 = AESCipher.T3_INV[Int(t[2] >> 24)]
            b[1] = b10 ^ b11 ^ b12 ^ b13
            
            let b20 = AESCipher.T0_INV[Int(t[2] & 0xFF)]
            let b21 = AESCipher.T1_INV[Int((t[1] >> 8) & 0xFF)]
            let b22 = AESCipher.T2_INV[Int((t[0] >> 16) & 0xFF)]
            let b23 = AESCipher.T3_INV[Int(t[3] >> 24)]
            b[2] = b20 ^ b21 ^ b22 ^ b23
            
            let b30 = AESCipher.T0_INV[Int(t[3] & 0xFF)]
            let b31 = AESCipher.T1_INV[Int((t[2] >> 8) & 0xFF)]
            let b32 = AESCipher.T2_INV[Int((t[1] >> 16) & 0xFF)]
            let b33 = AESCipher.T3_INV[Int(t[0] >> 24)]
            b[3] = b30 ^ b31 ^ b32 ^ b33
        }
    ...
    fileprivate func expandKey(_ key: Key) -> Array<Array<UInt32>> {
        ...
        var w = Array<UInt8>(repeating: 0, count: 4 * (AESBase.rounds + 1) * 4)
        for i in 0..<8 {
            for wordIdx in 0..<4 {
        ...
        for i in 8..<4 * (AESBase.rounds + 1) {
            ...
            for wordIdx in 0..<4 {
                w[4*i+wordIdx] = w[4*(i-8)+wordIdx]^tmp[wordIdx]
            }
        }
...
extension AESCipher {
    ...
    fileprivate func F1(_ x0: UInt32, _ x1: UInt32, _ x2: UInt32, _ x3: UInt32) -> UInt32 {
        var result: UInt32 = 0
        result |= UInt32(B1(AESCipher.T0[Int(x0 & 255)]))
        result |= UInt32(B1(AESCipher.T0[Int((x1 >> 8) & 255)])) << 8
        result |= UInt32(B1(AESCipher.T0[Int((x2 >> 16) & 255)])) << 16
        result |= UInt32(B1(AESCipher.T0[Int(x3 >> 24)])) << 24
        return result
    }
...
private struct CBCBlockMode {
    ...
    func encrypt(blocks: [[UInt8]], iv: [UInt8], cipherOperation: ([UInt8]) -> [UInt8]) -> [UInt8] {
        var out: [UInt8] = [UInt8]()
        out.reserveCapacity(blocks.count * blocks[blocks.startIndex].count)
        var prevCiphertext = iv
        for plaintext in blocks {
            prevCiphertext = cipherOperation(xor(plaintext, prevCiphertext))
            out.append(contentsOf: prevCiphertext)
        }
        return out
    }
    
    func decrypt(blocks: [[UInt8]], iv: [UInt8], cipherOperation: ([UInt8]) -> [UInt8]) -> [UInt8] {
        precondition(!blocks.isEmpty)
        var out: [UInt8] = [UInt8]()
        out.reserveCapacity(blocks.count * blocks[blocks.startIndex].count)
        var prevCiphertext = iv
        for ciphertext in blocks {
            out.append(contentsOf: xor(cipherOperation(ciphertext), prevCiphertext))
...
private struct PKCS7 {
    ...
    func remove(bytes: [UInt8], blockSize: Int?) -> [UInt8] {
        ...
        if finalLength < 0 { return bytes }
        if padding >= 1 { return Array(bytes[0..<finalLength]) }
        return bytes
    }
...
open class AESBase {
    ...
    static let rounds: Int = 14
    
    fileprivate static let Rcon: [UInt8] = [
